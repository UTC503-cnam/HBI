...
grandmother(X,Y) :- parent(Z,Y), parent(X,Z), female(Y).
brothers(X,Y) :- parent(Y,Z),parent(X,Z),male(X),male(Y),(X\=Y).
mother(X,Y) :- parent(X,Y),female(Y).
father(X,Y) :- parent(X,Y),male(Y).
son(X,Y) :- parent(X,Y),male(X).
daughter(X,Y) :- parent(X,Y),female(X).
siblings(X,Y) :- parent(Y,Z),parent(X,Z),(X\=Y).
aunt(X,Y) :- parent(X,A),parent(Y,B),siblings(A,B),(X\=Y).
